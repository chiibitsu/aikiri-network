    ...
    if bitcoin is not None:
        ...
        for b in blocks:
            ...
            if result == "complete":
                ...
                report.append(f"block {b.index}: Bitcoin proof not checked, {why}")
                pending += 1
            elif result == "pending":
                report.append(f"block {b.index}: Bitcoin proof pending, {why}")
                pending += 1
            else:
                report.append(f"block {b.index}: Bitcoin proof FAILED: {why}")
                return State.INVALID, report
